...
class AudioSet(Dataset):
    ...
    def get_label_dict(self):
        ...
        label_set, maxlen = set(), 0
        for labels in df.iloc[:, 1]:
            split = labels.split(',')
            maxlen = max(maxlen, len(split))
            for label in split:
                label_set.add(label)
        print(f'#Labels: {len(label_set)}, #Max Labels per Audio: {maxlen}')

        label_list = list(label_set)
        result = dict()
        for fn, labels in tqdm(df.iloc, desc='Extracting Labels'):
            ytid = fn[:11]
            lbs = labels.split(',')
            l = [0 for _ in range(len(label_set))]
            for lb in lbs:
                l[label_list.index(lb)] = 1
            result[ytid] = l
        return result, len(label_set)
